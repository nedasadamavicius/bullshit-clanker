defmodule BC.SandboxTest do
  use ExUnit.Case

  setup do
    # Create temporary directories for each test
    tmp_kb = System.tmp_dir!() |> Path.join("bc_test_kb_#{System.unique_integer()}")
    tmp_tree = System.tmp_dir!() |> Path.join("bc_test_tree_#{System.unique_integer()}")

    File.mkdir_p!(tmp_kb)
    File.mkdir_p!(tmp_tree)

    # Configure sandbox with test roots
    Application.put_env(:bc, :kb_root, tmp_kb)
    Application.put_env(:bc, :tree_root, tmp_tree)
  ...
  describe "resolve/2" do
    test "resolves valid relative paths", %{kb: kb} do
      ...
      assert {:ok, resolved} = BC.Sandbox.resolve(:kb, "test.txt")
      assert String.starts_with?(resolved, kb)
    end

    test "rejects absolute paths" do
      assert {:error, %{code: :outside_sandbox}} = BC.Sandbox.resolve(:kb, "/etc/passwd")
    end

    test "rejects paths with .. segments" do
      assert {:error, %{code: :outside_sandbox}} = BC.Sandbox.resolve(:kb, "../etc/passwd")
      assert {:error, %{code: :outside_sandbox}} = BC.Sandbox.resolve(:kb, "boards/../../etc")
    end

    test "rejects paths with NUL bytes" do
      assert {:error, %{code: :outside_sandbox}} = BC.Sandbox.resolve(:kb, "test\0file")
    end
    ...
    test "allows nested paths within root", %{kb: kb} do
      ...
      assert {:ok, resolved} = BC.Sandbox.resolve(:kb, "boards/x/board.toml")
      assert String.contains?(resolved, "boards/x/board.toml")
    end

    test "returns :no_tree error when tree not configured" do
      Application.put_env(:bc, :tree_root, nil)
      :persistent_term.erase(:tree_root)

      assert {:error, %{code: :no_tree}} = BC.Sandbox.resolve(:tree, "test.txt")
    end
  end
  ...
  describe "resolve/2 with symlinks" do
    test "follows symlinks within root", %{kb: kb} do
      ...
      assert {:ok, _resolved} = BC.Sandbox.resolve(:kb, "link/file.txt")
    end
    ...
    test "rejects symlinks pointing outside root", %{kb: kb} do
      ...
      case File.ln_s("/etc", link_path) do
        :ok ->
          assert {:error, %{code: :outside_sandbox}} = BC.Sandbox.resolve(:kb, "evil/passwd")
    ...
    test "detects symlink loops", %{kb: kb} do
      ...
      case File.ln_s(link2, link1) do
        :ok ->
          ...
          assert {:error, %{code: :symlink_loop}} = BC.Sandbox.resolve(:kb, "link1")
    ...
    test "root itself can be a symlink", %{kb: kb} do
      ...
      case File.ln_s(real_root, link_root) do
        :ok ->
          Application.put_env(:bc, :kb_root, link_root)
          :persistent_term.erase(:kb_root)

          assert {:ok, _resolved} = BC.Sandbox.resolve(:kb, "test.txt")
  ...
  describe "read/3" do
    test "reads text files", %{kb: kb} do
      ...
      assert {:ok, content} = BC.Sandbox.read(:kb, "test.txt")
    ...
    test "rejects files larger than max_bytes", %{kb: kb} do
      ...
      assert {:error, %{code: :too_large}} = BC.Sandbox.read(:kb, "large.txt")
    end
    ...
    test "allows custom max_bytes", %{kb: kb} do
      ...
      assert {:ok, _content} = BC.Sandbox.read(:kb, "medium.txt", max_bytes: 10_000)
      assert {:error, %{code: :too_large}} = BC.Sandbox.read(:kb, "medium.txt", max_bytes: 100)
    end

    test "rejects non-existent files" do
      assert {:error, %{code: :not_found}} = BC.Sandbox.read(:kb, "nonexistent.txt")
    end

    test "rejects directories" do
      File.mkdir_p!(Path.join(System.tmp_dir!(), "test_dir_#{System.unique_integer()}"))

      assert {:error, %{code: :not_a_file}} = BC.Sandbox.read(:kb, ".")
    end
    ...
    test "rejects binary files (contains NUL byte)", %{kb: kb} do
      ...
      assert {:error, %{code: :binary_file}} = BC.Sandbox.read(:kb, "binary.bin")
    end
    ...
    test "rejects invalid UTF-8", %{kb: kb} do
      ...
      assert {:error, %{code: :binary_file}} = BC.Sandbox.read(:kb, "invalid_utf8.txt")
    end
  end
  ...
  describe "list/3" do
    test "lists files in directory", %{kb: kb} do
      ...
      assert {:ok, entries} = BC.Sandbox.list(:kb, ".")
    ...
    test "respects entry limit", %{kb: kb} do
      ...
      assert {:ok, entries} = BC.Sandbox.list(:kb, ".", limit: 500)
    ...
    test "returns root-relative paths only", %{kb: kb} do
      ...
      assert {:ok, entries} = BC.Sandbox.list(:kb, "boards", depth: 2)
    ...
    test "rejects depth > 3" do
      assert {:error, %{code: :invalid_args}} = BC.Sandbox.list(:kb, ".", depth: 4)
    end

    test "rejects depth 0" do
      assert {:error, %{code: :invalid_args}} = BC.Sandbox.list(:kb, ".", depth: 0)
    end
  end

  describe "relative/2" do
    test "converts absolute path to root-relative", %{kb: kb} do
      assert {:ok, rel} = BC.Sandbox.relative(:kb, Path.join(kb, "test.txt"))
      assert rel == "test.txt"
    end

    test "handles nested paths", %{kb: kb} do
      assert {:ok, rel} = BC.Sandbox.relative(:kb, Path.join(kb, "boards/x/board.toml"))
      assert rel == "boards/x/board.toml"
    end

    test "rejects paths outside root" do
      assert {:error, %{code: :outside_sandbox}} = BC.Sandbox.relative(:kb, "/etc/passwd")
    end
  end

  describe "under?/2" do
    test "returns true for paths under root", %{kb: kb} do
      assert BC.Sandbox.under?(:kb, Path.join(kb, "test.txt"))
      assert BC.Sandbox.under?(:kb, Path.join(kb, "boards/x/board.toml"))
    end

    test "returns false for paths outside root" do
      assert not BC.Sandbox.under?(:kb, "/etc/passwd")
      assert not BC.Sandbox.under?(:kb, "/root/secret")
    end

    test "returns false when root not configured" do
      Application.put_env(:bc, :kb_root, nil)
      :persistent_term.erase(:kb_root)

      assert not BC.Sandbox.under?(:kb, "/some/path")
    end
  end
