defmodule BC.IngestTest do
  ...
  describe "ingest/2" do
    ...
    test "board.toml draft input produces Board with zero model calls" do
      ...
      fake_state = Application.get_env(:bc, :fake_model)
    ...
    test "markdown spec naming i.MX6UL, 0x80000000 RAM start, UART2 produces those fields" do
      ...
      Application.put_env(:bc, :fake_model, Fake.script(responses))
    ...
    test "CLI --spec PDF is rejected and points at mix bc.kb.ingest" do
      kb = Path.join(System.tmp_dir!(), "bc_pdf_kb_#{System.unique_integer([:positive])}")
      File.mkdir_p!(Path.join(kb, "boards"))
      pdf = Path.join(kb, "board.pdf")
      ...
      try do
        assert {:error, %{code: :unsupported, message: message}} =
                 BC.Config.from_argv(["--kb", kb, "--spec", pdf])

        assert message =~ "mix bc.kb.ingest"
      after
