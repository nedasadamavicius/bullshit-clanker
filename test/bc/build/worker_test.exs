defmodule BC.Build.WorkerTest do
  use ExUnit.Case, async: false

  describe "validation" do
    test "returns error when package starts with dash" do
      config = make_config("echo")
      start_supervised!({BC.Build.Worker, [config: config]})

      result = BC.Build.Worker.build("session123", package: "-invalid")
    ...
    test "returns error when timeout_ms is invalid" do
      ...
      result =
        BC.Build.Worker.build("session123", package: "github.com/example/pkg", timeout_ms: -1)
    ...
    test "handles missing toolchain" do
      ...
      result = BC.Build.Worker.build("session123", package: "test")
  ...
  describe "fake toolchain integration" do
    ...
    test "timeout handling" do
      ...
      result = BC.Build.Worker.build("session123", timeout_ms: 500, package: "test")
    ...
    test "broadcasts events" do
      ...
      # Subscribe to events
      Phoenix.PubSub.subscribe(BC.PubSub, "session:#{session_id}")

      result = BC.Build.Worker.build(session_id, package: "test")
  ...
  defp make_config(tamago_go_bin) do
    tree_root = Path.join(System.tmp_dir!(), "bc_test_tree_#{System.unique_integer()}")
    File.mkdir_p!(tree_root)

    %BC.Config{
      kb_root: Path.join(System.tmp_dir!(), "bc_test_kb_#{System.unique_integer()}"),
      tree_root: tree_root,
      model: "test-model",
  ...
  defp tmp_build_dir do
    dir = Path.join(System.tmp_dir!(), "bc_build_test_#{System.unique_integer()}")
    File.mkdir_p!(dir)
    dir
  end
