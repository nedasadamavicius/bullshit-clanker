defmodule BC.ToolsTest do
  use ExUnit.Case

  setup do
    # Create temporary directories for KB and tree
    tmp_kb = System.tmp_dir!() |> Path.join("bc_tools_test_kb_#{System.unique_integer()}")
    tmp_tree = System.tmp_dir!() |> Path.join("bc_tools_test_tree_#{System.unique_integer()}")
    ...
    on_exit(fn ->
      ...
      Application.delete_env(:bc, :kb_root)
      Application.delete_env(:bc, :tree_root)
    end)
  ...
  describe "schemas/1" do
    test "includes all required tools" do
      ...
      schemas = BC.Tools.schemas(config)
    ...
    test "excludes ws and tamago.build when tree_root is nil" do
      ...
      schemas = BC.Tools.schemas(config_no_tree)
      names = Enum.map(schemas, & &1["function"]["name"])
  ...
  describe "call/3 - kb.search" do
    test "requires at least one filter", %{ctx: ctx} do
      json_args = "{}"
      {:error, err} = BC.Tools.call("kb.search", json_args, ctx)
    ...
    test "returns empty result with note when no matches", %{ctx: ctx} do
      ...
      {:ok, result_json} = BC.Tools.call("kb.search", json_args, ctx)
    ...
    test "returns valid result structure", %{ctx: ctx} do
      ...
      case BC.Tools.call("kb.search", json_args, ctx) do
        {:ok, result_json} ->
          result = Jason.decode!(result_json)
  ...
  describe "call/3 - kb.read" do
    test "requires path argument", %{ctx: ctx} do
      json_args = "{}"
      {:error, err} = BC.Tools.call("kb.read", json_args, ctx)
      assert err.code == :invalid_args
    end
    ...
    test "returns wrapped content on success", %{kb: kb, ctx: ctx} do
      ...
      {:ok, result} = BC.Tools.call("kb.read", json_args, ctx)
    ...
    test "logs successful reads to read-log", %{kb: kb, ctx: ctx} do
      ...
      json_args = Jason.encode!(%{"path" => "test.txt"})
      {:ok, _result} = BC.Tools.call("kb.read", json_args, ctx)

      assert BC.Tools.ReadLog.read?(ctx.read_log, "test.txt")
    end

    test "does not log failed reads", %{ctx: ctx} do
      json_args = Jason.encode!(%{"path" => "nonexistent.txt"})
      {:error, _err} = BC.Tools.call("kb.read", json_args, ctx)

      assert not BC.Tools.ReadLog.read?(ctx.read_log, "nonexistent.txt")
    end
  end
  ...
  describe "call/3 - ws.read" do
    test "returns :no_tree when tree_root is nil", %{ctx: ctx} do
      ...
      json_args = Jason.encode!(%{"path" => "test.txt"})
      {:error, err} = BC.Tools.call("ws.read", json_args, ctx)
    ...
    test "returns wrapped content from tree", %{tree: tree, ctx: ctx} do
      ...
      json_args = Jason.encode!(%{"path" => "test.txt"})
      {:ok, result} = BC.Tools.call("ws.read", json_args, ctx)
  ...
  describe "call/3 - ws.list" do
    test "lists files in tree", %{tree: tree, ctx: ctx} do
      ...
      {:ok, result_json} = BC.Tools.call("ws.list", json_args, ctx)
      result = Jason.decode!(result_json)
  ...
  describe "call/3 - ws.diff" do
    test "returns :not_a_repo for non-git directories", %{tree: tree, ctx: ctx} do
      ...
      case BC.Tools.call("ws.diff", json_args, ctx) do
    ...
    test "works with git repository" do
      ...
      tmp_tree = System.tmp_dir!() |> Path.join("bc_git_test_#{System.unique_integer()}")
      File.mkdir_p!(tmp_tree)
      ...
      case System.cmd("git", ["init"], cd: tmp_tree) do
        {_output, 0} ->
          ...
          Application.put_env(:bc, :config, %BC.Config{
            kb_root: "/kb",
            tree_root: tmp_tree,
            model: "gpt-4",
            ingest_model: "gpt-4",
          ...
          json_args = "{}"
          {:ok, result} = BC.Tools.call("ws.diff", json_args, ctx)
  ...
  describe "call/3 - tamago.build" do
    test "rejects packages starting with dash", %{ctx: ctx} do
      ...
      {:error, err} = BC.Tools.call("tamago.build", json_args, ctx)
      assert err.code == :invalid_args
  ...
  describe "call/3 - unknown tool" do
    test "returns :unknown_tool with available list", %{ctx: ctx} do
      json_args = "{}"
      {:error, err} = BC.Tools.call("web_fetch", json_args, ctx)
