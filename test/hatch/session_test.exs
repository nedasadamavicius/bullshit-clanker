defmodule Hatch.SessionTest do
  use ExUnit.Case
  ...
  setup do
    ...
    Application.put_env(:hatch, :kb_boards, [])
  ...
  describe "send_message/2 with text-only response" do
    test "produces delta events, assistant message, and turn_finished", %{config: config} do
      ...
      Application.put_env(:hatch, :fake_model, Fake.script(fake_responses))
      ...
      # Wait for events
      assert_receive {:hatch_event, %{type: :user_message, text: "Hi there"}}, 1000
      assert_receive {:hatch_event, %{type: :assistant_delta, text: "Hello, world!"}}, 1000
      assert_receive {:hatch_event, %{type: :assistant_message, text: "Hello, world!"}}, 1000
      assert_receive {:hatch_event, %{type: :turn_finished, reason: :ok}}, 1000
  ...
  describe "send_message/2 with tool calls" do
    test "tool calls execute in order", %{config: config} do
      ...
      assert_receive {:hatch_event, %{type: :user_message}}, 1000

      # Wait for tool call started
      assert_receive {:hatch_event, %{type: :tool_call_started, name: "kb.search"}}, 2000

      # Wait for tool call finished
      assert_receive {:hatch_event, %{type: :tool_call_finished, name: "kb.search"}}, 1000

      # Wait for next model call, then the message
      assert_receive {:hatch_event, %{type: :assistant_message}}, 2000
      assert_receive {:hatch_event, %{type: :turn_finished, reason: :ok}}, 1000
  ...
  describe "cancel/1" do
    ...
    test "leaves transcript well-formed with matching tool messages when not mid-turn", %{config: config} do
      ...
      assert_receive {:hatch_event, %{type: :turn_finished}}, 2000
  ...
  describe "transcript/1" do
    test "remains responsive when model errors", %{config: config} do
      ...
      Application.put_env(:hatch, :model_client, ErrorModel)
      ...
      assert_receive {:hatch_event, %{type: :user_message}}, 1000
      assert_receive {:hatch_event, %{type: :error, stage: :model}}, 2000
      assert_receive {:hatch_event, %{type: :turn_finished, reason: :error}}, 1000
  ...
  describe "max steps" do
    test "stops at 12 steps with max_steps reason", %{config: config} do
      ...
      assert_receive {:hatch_event, %{type: :turn_finished, reason: :max_steps}}, 3000
  ...
  describe "two concurrent sessions" do
    test "run independently without sharing state", %{config: config} do
      Application.put_env(:hatch, :fake_model, Fake.script([{:text, "Response 1"}]))
