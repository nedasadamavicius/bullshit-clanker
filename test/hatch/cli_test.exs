defmodule Hatch.CLITest do
  ...
  describe "main/1" do
    ...
    test "exits 2 when --kb path does not exist" do
      assert_exit_2(fn ->
        Hatch.CLI.main(["--kb", "/does/not/exist"])
      end)
    end
    ...
    test "exits 2 when --tree path does not exist" do
      with_kb_dir(fn kb_dir ->
        assert_exit_2(fn ->
          Hatch.CLI.main(["--kb", kb_dir, "--tree", "/does/not/exist"])
        end)
      end)
    end
    ...
    test "exits 2 when required env vars are missing" do
      with_kb_dir(fn kb_dir ->
        # Save original env
        original_model = System.get_env("HATCH_MODEL")
        original_api_base = System.get_env("HATCH_API_BASE")
        original_api_key = System.get_env("HATCH_API_KEY")

        try do
          # Clear required env vars
          System.delete_env("HATCH_MODEL")
          System.delete_env("HATCH_API_BASE")
          System.delete_env("HATCH_API_KEY")

          assert_exit_2(fn ->
            Hatch.CLI.main(["--kb", kb_dir])
          end)
        after
          # Restore original env
          if original_model, do: System.put_env("HATCH_MODEL", original_model)
          if original_api_base, do: System.put_env("HATCH_API_BASE", original_api_base)
          if original_api_key, do: System.put_env("HATCH_API_KEY", original_api_key)
        end
      end)
    end
        ...
        with_kb_dir(fn kb_dir ->
          with_tmp_dir(fn tree_dir ->
            ...
            output =
              capture_io(fn ->
                try do
                  Hatch.CLI.main(["--kb", kb_dir, "--tree", tree_dir])
                catch
                  {:halted, 0} -> :ok
                end
              end)
