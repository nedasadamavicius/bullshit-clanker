defmodule Hatch.Acceptance do
  ...
  def run(opts \\ []) do
    ...
    previous_exclude = Application.get_env(:hatch, :search_exclude)
    Application.put_env(:hatch, :search_exclude, exclude)
    previous_applier = Application.get_env(:hatch, :patch_applier)
    # Real apply — the harness is the human. Do not use the test seam.
    Application.put_env(:hatch, :patch_applier, Hatch.Patch.Apply)

    try do
      config = build_config(mode, kb_root, tree_root, opts)
      Hatch.Config.put(config)
      Application.put_env(:hatch, :kb_root, kb_root)
      Application.put_env(:hatch, :tree_root, tree_root)
      ...
      {:ok, session_id, job_pid} = Hatch.BoardJob.Supervisor.start_session(config: config)
  ...
  defp ingest_draft(:scripted, spec_text, script) do
    ...
    Application.put_env(:hatch, :fake_model, Fake.script_many(ingest_turns))
  ...
  defp maybe_install_session_script(:scripted, script) do
    ...
    Application.put_env(:hatch, :fake_model, Fake.script_many(session_turns))
    :ok
  end
  ...
  defp resolve_proposal(session_id, job_pid, events) do
    store = Hatch.BoardJob.Supervisor.get_proposal_store(job_pid)
  ...
  defp maybe_apply(proposal, session_id, tree_root) do
    # Harness is the operator (spec 013). Live entry is mix hatch.accept;
    ...
    case Hatch.Patch.Apply.apply_patch(proposal, permit, tree_root) do
  ...
  defp maybe_build(session_id, config, draft, true) do
    ...
    case Hatch.Build.Worker.build(session_id,
           package: "./...",
  ...
  defp ensure_builder(config) do
    ...
    {:ok, _pid} = Hatch.Build.Worker.start_link(config: config)
    :ok
  end

  defp restore_env(key, nil), do: Application.delete_env(:hatch, key)
  defp restore_env(key, value), do: Application.put_env(:hatch, key, value)
end
