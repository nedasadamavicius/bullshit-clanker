defmodule Hatch.TUI.Model do
  ...
  # PubSub events wrapped in hatch_event
  def update(state, {:hatch_event, event}) do
    handle_hatch_event(state, event)
  end
  ...
  defp handle_insert_key(state, :enter) do
    ...
    Hatch.Session.send_message(state.session_id, text)
  ...
  defp handle_normal_key(state, :char_b) do
    ...
    Hatch.Session.send_message(state.session_id, "tools.build()")
    state
  end

  defp handle_normal_key(state, :char_c) do
    # Cancel the running turn
    Hatch.Session.cancel(state.session_id)
    state
  end
    ...
    case state.confirm_state do
        ...
        case state.pending_proposal do
          ...
          proposal ->
            ...
            unless tree_root do
              ...
              applier = Application.get_env(:hatch, :patch_applier, Hatch.Patch.Apply)

              Task.start(fn ->
                case applier.apply_patch(proposal, permit, tree_root) do
                  {:ok, result} ->
                    Hatch.Session.note_applied(state.session_id, proposal, result[:output])

                  {:error, err} ->
                    Hatch.Session.note_applied(state.session_id, proposal, err[:message])
                end
              end)
      ...
      {:pending_reject, proposal_id} ->
        ...
        case state.pending_proposal do
          ...
          proposal ->
            Hatch.Session.note_rejected(state.session_id, proposal, "")
            %{state | confirm_state: :none, status_message: nil}
        end
  ...
  defp handle_hatch_event(state, %{type: :user_message, text: text}) do
    new_transcript = state.transcript ++ ["user: #{text}"]
    %{state | transcript: new_transcript}
  end

  defp handle_hatch_event(state, %{type: :assistant_delta, text: text}) do
  ...
  defp handle_hatch_event(state, %{type: :assistant_message, text: text}) do
    new_transcript = state.transcript ++ ["assistant: #{text}"]
    %{state | transcript: new_transcript}
  end

  defp handle_hatch_event(state, %{
  ...
  defp handle_hatch_event(state, %{type: :build_started, build_id: _build_id, argv: _argv}) do
    %{state | build_state: :building}
  end

  defp handle_hatch_event(state, %{type: :build_log, build_id: _build_id, chunk: _chunk}) do
    state
  end
  ...
  defp handle_hatch_event(state, %{type: :turn_finished, reason: _reason}) do
    state
  end

  defp handle_hatch_event(state, %{type: :error, stage: stage, message: message}) do
    new_transcript = state.transcript ++ ["error (#{stage}): #{message}"]
    %{state | transcript: new_transcript}
  end

  defp handle_hatch_event(state, _event) do
