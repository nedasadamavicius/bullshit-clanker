defmodule BC.Patch.Apply do
  ...
  @spec apply_patch(BC.Proposal.t(), BC.Permit.t(), String.t()) ::
  ...
  defp verify_permit(proposal, permit) do
    ...
    if permit.proposal_id != proposal.id do
      ...
      if permit.patch_hash != BC.Proposal.patch_hash(proposal) do
        {:error, :stale}
      else
        # Consume the permit (checks expiry and spent status)
        BC.Permit.consume(permit)
      end
    end
  end
  ...
  defp do_apply(proposal, tree_root) do
    ...
    with {:ok, parsed_files} <- BC.Proposal.Patch.parse(proposal.patch),
  ...
  defp validate_sandbox(parsed_files, tree_root) do
    ...
    Enum.reduce_while(parsed_files, :ok, fn file, _acc ->
      ...
      case BC.Sandbox.under?(:tree, full_path) do
        true ->
          {:cont, :ok}
