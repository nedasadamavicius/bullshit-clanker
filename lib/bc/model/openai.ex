defmodule BC.Model.OpenAI do
  ...
  def chat(messages, opts, stream_callback) do
    ...
    tools = opts[:tools] || []
    temperature = cap_temperature(opts[:temperature] || 1.0, config)
    max_tokens = opts[:max_tokens] || default_max_tokens(config)
  ...
  defp cap_temperature(temp, config) do
    if BC.Provider.anthropic?(config) do
      min(temp, 1.0)
    else
      temp
    end
  ...
  defp default_max_tokens(config) do
    if BC.Provider.anthropic?(config), do: 8192, else: nil
  end
