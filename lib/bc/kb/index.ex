defmodule BC.KB.Index do
  ...
  def get(board_id) do
    case :ets.lookup(:bc_kb, board_id) do
  ...
  @impl true
  def init(_opts) do
    :ets.new(:bc_kb, [:named_table, :protected, read_concurrency: true])
  ...
  def handle_call(:ensure_built, _from, state) do
    ...
    :ets.delete_all_objects(:bc_kb)

    Enum.each(boards, fn board ->
      :ets.insert(:bc_kb, {board.id, board})
    end)
