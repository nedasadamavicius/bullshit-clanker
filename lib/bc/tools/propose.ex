defmodule BC.Tools.Propose do
  ...
  alias BC.KB.Index
  alias BC.Proposal
  alias BC.Proposal.Patch
  alias BC.Proposal.Citations
  ...
  @spec propose(map(), BC.Tools.ctx()) :: {:ok, String.t()} | {:error, err()}
  ...
  @spec check_board_exists(String.t()) :: {:ok, BC.KB.Board.t()} | {:error, err()}
  ...
  @spec check_patch_sandbox([Patch.parsed_file()], BC.Tools.ctx()) :: :ok | {:error, err()}
  ...
  defp build_deltas(%BC.KB.Board{} = draft, %BC.KB.Board{} = board, _raw) do
    BC.KB.Delta.compare(draft, board)
  end
  ...
  defp delta_from_model(row) when is_map(row) do
    ...
    cond do
      ...
      is_atom(field) and not is_nil(field) ->
        %BC.KB.Delta{field: field, draft: to, board: from, kind: :differs}
  ...
  defp broadcast_proposal(ctx, proposal) do
    ...
    PubSub.broadcast(BC.PubSub, "session:#{ctx.session_id}", {:bc_event, event})
    :ok
  end
