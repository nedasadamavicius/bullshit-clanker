defmodule BC.Tools.KB do
  ...
  def search(args, ctx) do
    ...
    with :ok <- validate_search_args(soc, uart, peripherals, text, limit),
      ...
      case BC.KB.Search.search(query) do
        {:ok, hits} ->
  ...
  defp search_exclude(args, _ctx) do
    ...
    from_env = List.wrap(Application.get_env(:bc, :search_exclude, []))
  ...
  defp build_search_result([], soc) do
    note =
      if soc do
        "no board in the KB has this soc. bc cannot propose a port for an soc with no package in the KB."
      else
