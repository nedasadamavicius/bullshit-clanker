defmodule BC.Config do
  ...
  @spec put(t()) :: :ok
  def put(config) do
    :persistent_term.put({:bc, :config}, config)
  end

  @spec get() :: t()
  def get do
    :persistent_term.get({:bc, :config})
  end
  ...
  defp load_env({argv_opts, env_map}) do
    ...
    with :ok <- validate_kb(kb),
         ...
         {:ok, resolved} <- BC.Provider.resolve(env_map) do
      build_model = Map.get(env_map, "BC_MODEL_BUILD")
      tamago_go = Map.get(env_map, "BC_TAMAGO_GO", "tamago-go")

      build_timeout_ms =
        case Map.get(env_map, "BC_BUILD_TIMEOUT_MS", "120000") do
    ...
    if String.ends_with?(expanded, ".pdf") or String.ends_with?(expanded, ".PDF") do
      {:error,
       %{
         code: :unsupported,
         message:
           "PDFs are ingested offline into board.toml. Run: mix bc.kb.ingest --kb PATH --pdf #{spec_path}"
       }}
    else
