defmodule BC.Acceptance.Fixture do
  @moduledoc """
  ...
  def prepare_tree(source \\ tree_path()) do
    scratch =
      Path.join(System.tmp_dir!(), "bc_accept_tree_#{System.unique_integer([:positive])}")
  ...
  def prepare_kb(source \\ kb_path()) do
    scratch =
      Path.join(System.tmp_dir!(), "bc_accept_kb_#{System.unique_integer([:positive])}")
  ...
  def git_init!(dir) do
    ...
    {_, 0} = System.cmd(git, ["config", "user.email", "bc@example.test"], cd: dir)
    {_, 0} = System.cmd(git, ["config", "user.name", "BC"], cd: dir)
